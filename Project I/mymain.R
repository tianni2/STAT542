...
# construct the function to capture the variables which has missing values
missing.n=sapply(names(test.x),
                 function(x)
                   length(which(is.na(test.x[,x]))))
# get the variable name
id=which(is.na(test.x$Garage_Yr_Blt))
...
colnames(elastic)=c("PID", "Sale_Price")
write.table(elastic,"mysubmission1.txt",row.names = FALSE,quote = FALSE, sep = ", ")

Sale_Price=exp(xgboost.pred)
boost=cbind(PID,Sale_Price)
colnames(boost)=c("PID", "Sale_Price")
write.table(boost,"mysubmission2.txt",row.names = FALSE,quote = FALSE, sep = ", ")
